...
if terraform_location_env != False and len(sys.argv) > 1 and sys.argv[1] == '--list':
    ...
    inventory_json = {"all": {"children": ["app", "db", "ungrouped", "localhost1"]}, "localhost1": {"hosts": ["localhost"]},"app": {"hosts": ["appserver"]}, "db": {"hosts": ["dbserver"]}, "_meta": {"hostvars": {"appserver": {"ansible_host": terraform_state_json["outputs"]["external_ip_address_app"]["value"], "db_host": terraform_state_json["outputs"]["internal_ip_address_db"]["value"]}, "dbserver": {"ansible_host": terraform_state_json["outputs"]["external_ip_address_db"]["value"]}, "localhost1":{"ansible_host": "127.0.0.1" }}}}
